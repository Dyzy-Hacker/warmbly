...
In the **Invite teammates** box, paste or type addresses into **Emails**, separated by comma, semicolon, space, or Enter. Each becomes a chip: valid ones grey, malformed ones red so you can remove them first. Pick a **Role** for the batch (a description on the right says what you are granting), then **Send invitations**.

Warmbly sends one invitation per address and reports results as "Invited 2 · 1 failed". Invalid addresses are skipped and called out.

**Pending invitations** shows the email, role, and expiry for each unaccepted invite, with **Copy invite link** (to share directly instead of waiting on email) and **Cancel invitation**. Expired invites need re-sending.
...
Each row shows email, role, and join date, with your own row tagged "you". Click a role pill to change it and confirm; per-role counts update immediately.
