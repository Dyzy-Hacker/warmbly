...
| Organization | Sunrise Labs (Pro plan) |
...
- the Sunrise Labs org on an active Pro subscription, owned by `sandbox@warmbly.test`, plus a second teammate (Marco Diaz, admin), one pending invitation, and an active developer API key
...
- `liam.walsh@sunrise.test` has a month of real sending behind it (1,600 completed tasks) with 72 bounces and one complaint, putting it at `4.5%` bounce and `0.06%` complaint. Both come from real `tasks` and `deliverability_events` rows, so the Advisor's numbers reconcile with the Deliverability page rather than contradicting it. Its cap is also raised to 120/day.
- `yuki.sato@sunrise.test` has SPF and DKIM but no DMARC, and a 90-second gap between sends.
- `erik.lund@sunrise.test` was connected six days ago, went straight to the full 50/day cap, and has warmup paused.
